...
fn (mut corp Corp) update(mut app App, cible Vector){
	...
	corp.left_arm.update(mut app, cible + Vector{x: -10})
	corp.right_arm.update(mut app, cible + Vector{x: 10})
...
fn (corp Corp) is_gravity(app App) bool{
	for id in corp.body_anchor_index{
...
fn (mut corp Corp) ajustement(mut app App){
	...
	if corp.left_leg.end_pos.x < corp.pos.x -40{
		corp.left_leg.end_pos	= corp.left_leg.start_pos	+ Vector{x: 40, y: 80}
	}
	else if corp.left_leg.end_pos.x > corp.pos.x +40{
		corp.left_leg.end_pos	= corp.left_leg.start_pos	+ Vector{x: -40, y: 80}
	}
	
	if corp.right_leg.end_pos.x < corp.pos.x -40{
		corp.right_leg.end_pos	= corp.right_leg.start_pos	+ Vector{x: 40, y: 80}
	}
	else if corp.right_leg.end_pos.x > corp.pos.x +40{
		corp.right_leg.end_pos	= corp.right_leg.start_pos	+ Vector{x: -40, y: 80}
	}
}
