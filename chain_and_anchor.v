...
pub fn front_go_to(mut points []vec.Vec2[f64], pos_constraints []f64, angle_constraints []f64, target vec.Vec2[f64], repetition int){
	for _ in 0 .. repetition{
		...
		exclude_front_to_back(mut points, pos_constraints, angle_constraints)
		println('pt: $points')
	}
}

pub fn exclude_front_to_back(mut points []vec.Vec2[f64], pos_constraints []f64, angle_constraints []f64) {
	for ip in 0 .. points.len - 2 {
		 points[ip+1] = apply_pos_constraint(points[ip], points[ip + 1], pos_constraints[ip])
		points[ip+1] = apply_angle_constraint(points[ip], points[ip + 1], points[ip + 2],
			angle_constraints[ip])
	}
...
pub fn extend_front_to_back_min_max(mut points []vec.Vec2[f64], pos_constraints []f64, min_constraints []f64, max_constraints []f64) {
        for ip in 0 .. points.len - 2 {
               
                points[ip + 1] = apply_min_max_angle_constraint(points[ip], points[ip + 1], points[ip + 2],
                        min_constraints[ip], max_constraints[ip])
        }
        points[points.len - 1] = apply_pos_constraint(points[points.len - 2], points[points.len - 1],
                pos_constraints[points.len - 2])
